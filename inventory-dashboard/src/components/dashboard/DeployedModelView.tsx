...
export default function DeployedModelView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true)
        const res = await fetch('http://127.0.0.1:8000/getDeployedModelView', {
          method: 'POST',
          headers: {
        ...
        console.error('Failed to load deployed model view', e)
        setError('Failed to load deployed model view')
      } finally {
        setLoading(false)
      }
    }
