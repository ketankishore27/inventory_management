...
export default function StockModelSubStatus() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true)
        const res = await fetch('http://127.0.0.1:8000/getStockModelSubStatus', {
          method: 'POST',
          headers: {
        ...
        console.error('Failed to load stock model substatus', e)
        setError('Failed to load stock model substatus')
      } finally {
        setLoading(false)
      }
    }
