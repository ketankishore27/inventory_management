...
export default function DeployedModelSubStatus() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true)
        const res = await fetch('http://127.0.0.1:8000/getDeployedModelSubStatus', {
          method: 'POST',
          headers: {
        ...
        console.error('Failed to load deployed model substatus', e)
        setError('Failed to load deployed model substatus')
      } finally {
        setLoading(false)
      }
    }
    ...
    <ResponsiveContainer width="100%" height="100%">
      <BarChart data={data} margin={chartMargin}>
        ...
        <Legend align="right" verticalAlign="top" content={() => (
          <ChartLegendContent items={[{ label: 'Deployed', color: ChartColors.deployed }]} />
        )} />
        <Tooltip {...defaultTooltipProps} formatter={(v) => formatNumber(Number(v))} />
        <Bar dataKey="value" fill={ChartColors.deployed} radius={barRadius}>
          <ValueLabels />
        </Bar>
      </BarChart>
    </ResponsiveContainer>
